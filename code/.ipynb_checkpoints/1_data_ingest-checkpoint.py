...
if os.environ["STORAGE_MODE"] == "external":
    path = f"{storage}/{data_location}/loan_data.csv"
else:
...
if os.environ["STORAGE_MODE"] == "external":
    ...
    try:
        data = spark.read.csv(path, header=True, schema=schema, sep=",", nullValue="NA")
        ...
        data.coalesce(1).write.csv(
            "file:/home/cdsw/raw/data/", mode="overwrite", header=True
        )
        ...
        if hive_table not in list(
            ...
            try:
                data.write.format("parquet").mode("overwrite").saveAsTable(
                    hive_table_fq
                )
