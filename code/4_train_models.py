...
#
# These three components are provided for the loan eligibility model as `4_train_models.py`, `requirements.txt`,
...
try:
  ...
  pass
from eligibility_explainer import ExplainedModel, CategoricalEncoder
...
try:
    ...
    df = pd.read_csv(os.path.join("../raw", "loan_data.csv"))
...
df = (df
      ...
      .drop(columns=['index', 'Loan_ID'])
     )
try:
    # when loading from external data source, this column has str dtype
    df.replace({"CreditHistory": {"1": "Yes", "0": "No"}}, inplace=True)
except:
